...
- Gestión eficiente de archivos CSV grandes (60+ MB, 200k+ registros)
- Procesamiento ultra rápido mediante vroom
...
├── contrato_farmacia.json                  # Contrato de datos (Esquema)
├── R/                                     # Módulos funcionales (Granularidad)
│   ├── cleaning.R                         # Limpieza y estandarización
│   ├── parsing.R                          # Parseo de fechas y horas
│   ├── features.R                         # Variables derivadas
│   └── validation.R                       # Hashes y balance de masas
├── src/                                   # Orquestación
│   ├── etl/
│   │   └── etl_bigdata.R                  # Orquestador principal modular
│   └── utils/
│       └── logging.R                      # Sistema de logging
├── tests/                                 # Suite de pruebas
│   ├── testthat/                          # Pruebas unitarias
│   └── run_tests.R                        # Ejecutor de pruebas
├── datos/                                 # Almacén de Datos (Medallion)
│   ├── entrada/                           # CSVs crudos originales
│   ├── bronze/                            # Copias fieles Parquet
│   ├── silver/                            # Datos normalizados Zstd
│   └── gold/                              # Reporte final consolidado
├── schemas/                               # Contratos y Esquemas (JSON)
├── diccionario_homologacion/               # Diccionario iGLOBAL
...
install.packages(c(
  ...
  "readxl",      # Lectura de diccionario XLSX
  "vroom",       # Lectura ultra rápida de Big Data
...
- Ubicación: carpeta `datos/entrada/`
- Estructura: Encabezados en fila 8, datos desde fila 9
- Primeras 7 filas: Metadatos (se saltan automáticamente)
...
2. Colocar archivos CSV en carpeta `datos/entrada/`
...
### PASO 0: Validación de Directorios (Medallion)
- Verifica existencia de `datos/entrada/` (fuente de datos recursiva)
- Verifica existencia de `datos/bronze/`, `datos/silver/` y `datos/gold/`
- Verifica existencia de `schemas/` (contrato de datos)
- Registra auditoría de inicio en `logs/`
...
1. **Formato de entrada:** Solo archivos CSV delimitados por ; (no XLSX, no XLS)
2. **Estructura de encabezados:** Debe estar en fila 8 exactamente
